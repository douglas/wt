...
func init() {
	registerCommand(&command{
		...
		run: func(_ []string) error {
			if isJSONOutput() {
				...
				return emitJSONSuccess("list", map[string]any{"worktrees": entries})
			}
	...
	removeFlagSet := flag.NewFlagSet("remove", flag.ContinueOnError)
	removeFlagSet.BoolVar(&removeForce, "force", false, "Force removal even if worktree has modifications")
	removeFlagSet.BoolVar(&removeForce, "f", false, "Force removal even if worktree has modifications")
	...
	registerCommand(&command{
		...
		run: func(args []string) error {
			...
			if jsonMode {
				return emitJSONSuccess("remove", map[string]any{
					"status":      "removed",
					"branch":      branch,
					"path":        existingPath,
	...
	cleanupFlagSet := flag.NewFlagSet("cleanup", flag.ContinueOnError)
	cleanupFlagSet.BoolVar(&cleanupDryRun, "dry-run", false, "Preview what would be removed")
	cleanupFlagSet.BoolVar(&cleanupForce, "force", false, "Remove all without confirmation")
	cleanupFlagSet.BoolVar(&cleanupForce, "f", false, "Remove all without confirmation")
...
Examples:
  ...
  wt cleanup --force      # Remove all without confirmation`,
		...
		run: func(_ []string) error {
			...
			if len(toRemove) == 0 {
				if jsonMode {
					return emitJSONSuccess("cleanup", map[string]any{"removed": 0, "skipped": 0, "base": base, "worktrees": []string{}})
				}
			...
			if cleanupDryRun {
				if jsonMode {
					...
					return emitJSONSuccess("cleanup", map[string]any{"dry_run": true, "base": base, "worktrees": planned})
				}
			...
			if jsonMode {
				return emitJSONSuccess("cleanup", map[string]any{"dry_run": false, "base": base, "removed": removed, "skipped": skipped})
			}
	...
	registerCommand(&command{
		...
		run: func(_ []string) error {
			...
			if isJSONOutput() {
				return emitJSONSuccess("prune", map[string]any{"status": "pruned"})
			}
